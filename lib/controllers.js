...
var controllers = function () {
  ...
  for (var ctrl in modules.controllers) {
    ...
    for (i = 0, z = controller.data.length; i<z; i++) {
      ...
      data[dataRef] = new modules.adapters[config.db.adapter](dataRef, config.db.config);
    ...
    sockets = {};
    if (controller.hasOwnProperty('sockets') && controller.sockets.length && modules.hasOwnProperty('socketio')) {
      for (i=0, z=controller.sockets.length; i<z; i++) {
        socketRef = controller.sockets[i];
        sockets[socketRef] = modules.socketio.getNamespace(socketRef);
      }
