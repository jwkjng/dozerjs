...
var api = {
  ...
  process: function (req, res) {
    ...
    var reqPath = req.params[0];// everything after the entity
    var supportedMethods = [];
    var supportedRoutes = {};
    var paramNames = [];
    var method, endpointMethod, fn, controller, matcher, z;

    // Verify entity exists or return 404
    if (!modules.api.hasOwnProperty(entity)) {
      res.send(404, { 'status': 'error', 'message': 'entity does not exist' });
      return;
    }
    ...
    for (method in modules.api[entity]) {
    ...
    endpointMethod = modules.api[entity][reqMethod]; // e.g. '/api/user/GET'

    // Check if endpointMethod is array (has multiple routes defined)
    if(Array.isArray(endpointMethod)){
      ...
      // Set entity method to false while matching
      ...
      for (var route in supportedRoutes) {
        ...
        if (matcher !== null) {
          ...
          // Set entity method to matched route
      ...
      if (!endpointMethod) {
        endpointMethod = supportedRoutes.index;
      }
    ...
    if (!modules.controllers.hasOwnProperty(entity)) {
      res.send(500, { 'status': 'error', 'message': 'Controller error - Does Not Exist' });
      return;
    }

    // Set controller variable
    controller = modules.controllers[entity];

    // Verify controller entity method exists and is function or return 500
