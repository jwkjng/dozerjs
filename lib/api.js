    ...
    if(Object.prototype.toString.call(endpointMethod) === '[object Array]'){
      ...
      for (var route in supportedRoutes) {
        ...
        if (matcher !== null) {
          ...
          var names = [];
          supportedRoutes[route].path.replace(/:(\w+)/g, function(_, match) {
            names.push(match);
          });
          ...
          if (matcher.length > 1) {
            for (i = 1, z = matcher.length; i < z; i++) {
              req.params[names[i-1]] = matcher[i];
            }
          }
          console.log("Matched " + supportedRoutes[route].path + ' WITH: '+ JSON.stringify(params));
